...
export default function AdminOrdersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {view === "grid" ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{orders.map((order) => <article key={order.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><h2 className="font-semibold text-slate-900">{order.id}</h2><span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{order.status}</span></div><p className="mt-4 text-sm text-slate-600">{order.customerName}</p><p className="text-sm text-slate-500">{order.shopName}</p><div className="mt-5 flex justify-between border-t border-slate-100 pt-3 text-sm"><span className="text-slate-500">{order.date}</span><strong>${order.total}</strong></div></article>)}</div> : <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
