...
import { getSellerProducts } from "@/lib/api/admin";
...
export default function SellerMyListingsPage() {
  ...
  const [items, setItems] = useState<SellerProductDto[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const result = await getSellerProducts(profile.id, 1, 100);
  ...
  const filtered = items.filter((item) => item.productName.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="relative max-w-md flex-1">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm text-slate-700 outline-none focus:border-[var(--brand)] focus:bg-white" placeholder="Search my listings" />
          </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-slate-500">
          You haven&apos;t added any products to your listings yet. Go to Available Products and use &quot;Add to My Listings&quot;.
        </div>
      ) : view === "grid" ? (
