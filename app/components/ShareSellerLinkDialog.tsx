...
function buildLink(invite: string) {
...
export function ShareSellerLinkDialog({ onClose, initialInvite = "" }: { onClose: () => void; initialInvite?: string }) {
  ...
  const link = useMemo(() => buildLink(invite), [invite]);
  const message = `You're invited to sell on WayFair. Register here: ${link}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onClick={onClose}>
      <div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" role="dialog" aria-modal="true" aria-label="Share seller registration link" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-3 text-lg font-semibold text-slate-900">Share seller registration link</h2>
            <p className="mt-1 text-sm text-slate-500">Send this link to a seller. It opens the registration form, and once approved they sign in on the normal login page.</p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <a href={`mailto:?subject=${encodeURIComponent("Sell on WayFair")}&body=${encodeURIComponent(message)}`} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
            <Mail className="h-4 w-4 text-sky-600" /> Email
          </a>
          {canNativeShare && (
            <button onClick={() => navigator.share({ title: "Sell on WayFair", text: "You're invited to sell on WayFair.", url: link }).catch(() => {})} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
